...
function defineStore(definition) {
    var store = defineDisposable(null, function (c) {
        ...
        definition.catches.forEach(function (e) { return e.on(changed); });
        return {
            instance: instance,
            destructor: destructor
        };
        ...
        function destructor() {
            definition.catches.forEach(function (e) { return e.off(changed); });
    ...
    if (definition.queries)
        Object.keys(definition.queries).forEach(function (q) {
            definition.queries[q].set_store(store);
        });
    store.query = definition.queries;
    return store;
}
